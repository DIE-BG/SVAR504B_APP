...
%% short
toplot = {'ln_v_sa', 'ln_cpi_sub_sa', 'ln_y_sa', 'ln_bm_sa'};
MODEL.PostProc_temp = dbclip(MODEL.PostProc, MODEL.DATES.hist_end-20,...
                            MODEL.DATES.hist_end +20);
post_proc_tem = dbclip(post_proc, MODEL.DATES.hist_end-20,...
                            MODEL.DATES.hist_end +20);
...
for i = 1:length(toplot)
    ...
    %titulos
    title(regexprep(MODEL.PostProc_temp.l_sa.(toplot{i}).Comment{1}, '\([^)]*\)', ''),...
        '(niveles)','Interpreter', 'none');
    ...
    if i == 4
       SimTools.scripts.pausaGuarda(fullfile('plots',...
                                      MODEL.CORR_DATE, MODEL.CORR_VER,...
                                      'otras',...
                                      'Velocidad (componentes)short.png'), ...
                       'AutoSave', true) 
    end
