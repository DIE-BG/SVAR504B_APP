...
MODEL.PostProc = struct();

%% Logaritmos

temp_db = databank.copy(MODEL.F_pred, params.list); 
% Logaritmos desestacionalizados
MODEL.PostProc.l_sa = databank.apply(@(x) x12(x), temp_db,...
                    'StartsWith=','ln_',...
                    'Append=', '_sa',...
                    'RemoveSource=',true);

% Tendencias HP de los logs desestacionalizados           
MODEL.PostProc.l_bar = databank.apply(@(x) hpf(x), MODEL.PostProc.l_sa,...
                    'EndsWith=','_sa',...
                    'RemoveEnd=',true,...
                    'Append=', '_bar',...
                    'RemoveSource=',true);
...
for i = 1:length(params.list)
   MODEL.PostProc.l_gap.(strcat(params.list{i},'_gap')) = ...
                        MODEL.PostProc.l_sa.(strcat(params.list{i},'_sa')) - ...
                        MODEL.PostProc.l_bar.(strcat(params.list{i},'_bar'));   
end
%% Niveles
temp_db = databank.copy(MODEL.F_pred, params.list_niv); 

% Niveles 
MODEL.PostProc.niv = databank.apply(@(x) exp(x/100), temp_db,...
                    'StartsWith=','ln_',...
                    'RemoveStart=', true,...
                    'RemoveSource=',true);
% Niveles desestacionalizados
MODEL.PostProc.niv_sa = databank.apply(@(x) x12(x), MODEL.PostProc.niv,...
                    'Append=', '_sa',...
                    'RemoveSource=',true);                
% Tendencias HP de los niveles desestacionalizados  
MODEL.PostProc.niv_bar = databank.apply(@(x) hpf(x), MODEL.PostProc.niv_sa,...
                    'EndsWith=','_sa',...
                    'RemoveEnd=',true,...
                    'Append=', '_bar',...   
                    'RemoveSource=',true);
